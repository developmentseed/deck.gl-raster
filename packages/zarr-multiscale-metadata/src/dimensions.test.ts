...
describe("identifySpatialDimensions", () => {
  ...
  it("uses override for lat dimension", () => {
    const result = identifySpatialDimensions(["time", "custom_lat", "lon"], {
      lat: "custom_lat",
    });
  ...
  it("uses override for lon dimension", () => {
    const result = identifySpatialDimensions(["lat", "custom_lon", "time"], {
      lon: "custom_lon",
    });
  ...
  it("override is case insensitive", () => {
    const result = identifySpatialDimensions(["MY_LAT", "MY_LON"], {
      lat: "my_lat",
      lon: "my_lon",
    });
