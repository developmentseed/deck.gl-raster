...
describe("detectMultiscaleFormat", () => {
  it("detects zarr-conventions format from layout array", () => {
    expect(detectMultiscaleFormat(zarrConventionsMultiscale)).toBe(
      "zarr-conventions",
    );
  });
  ...
  it("detects ndpyramid tiled format from pixels_per_tile", () => {
    expect(detectMultiscaleFormat(ndpyramidTiledMultiscale)).toBe(
      "ndpyramid-tiled",
    );
  });
...
describe("parseZarrConventions", () => {
  it("extracts level paths from layout", () => {
    const result = parseZarrConventions(
      zarrConventionsMultiscale,
      "temperature",
      null,
    );
  ...
  it("extracts spatial:transform and spatial:shape", () => {
    ...
    expect(result.levels[0].spatialTransform).toEqual([
      0.1, 0, -180, 0, -0.1, 90,
    ]);
  ...
  it("extracts shape/chunks from consolidated metadata", () => {
    const consolidatedMeta =
      v3GroupMetadata.consolidated_metadata?.metadata ?? null;
    const result = parseZarrConventions(
      zarrConventionsMultiscale,
      "temperature",
      consolidatedMeta,
    ...
    const customOmeNgff: typeof omeNgffMultiscale = [
      {
        datasets: [
          ...
          {
            path: "1",
            coordinateTransformations: [{ type: "scale", scale: [3.0, 3.0] }],
          }, // 3x, not 2x
        ],
      },
    ];
...
describe("Data transforms extraction", () => {
  it("extracts scale_factor and add_offset from consolidated metadata", () => {
    const consolidatedMeta =
      v3GroupWithDataTransforms.consolidated_metadata?.metadata ?? null;
    const multiscales = v3GroupWithDataTransforms.attributes
  ...
  it("does not set scaleFactor/addOffset when not in metadata", () => {
    const consolidatedMeta =
      v3GroupWithFloatData.consolidated_metadata?.metadata ?? null;
    const multiscales = v3GroupWithFloatData.attributes
      ?.multiscales as ZarrConventionsMultiscale;
    const result = parseZarrConventions(multiscales, "data", consolidatedMeta);
...
describe("Spec-compliant fixtures", () => {
  ...
  it("parses spec-compliant zarr-conventions with proper transform structure", () => {
    const multiscales = v3GroupMetadataSpecCompliant.attributes
      ?.multiscales as ZarrConventionsMultiscale;
