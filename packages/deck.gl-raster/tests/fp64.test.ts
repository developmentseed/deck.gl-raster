...
import { splitFloat64 } from "../src/fp64.js";

describe("splitFloat64", () => {
  it("returns [low, high] Float32Arrays of the same length", () => {
    const [low, high] = splitFloat64(new Float64Array([1, 2, 3]));
  ...
  it("high part is Math.fround of each value", () => {
    ...
    const [, high] = splitFloat64(values);
  ...
  it("high + low reconstructs the original to < 1e-6 m at high magnitude", () => {
    ...
    const [low, high] = splitFloat64(values);
    for (let i = 0; i < values.length; i++) {
  ...
  it("handles an empty array", () => {
    const [low, high] = splitFloat64(new Float64Array([]));
