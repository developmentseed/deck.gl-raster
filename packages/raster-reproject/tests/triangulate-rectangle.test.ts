...
import { RasterReprojector, triangulateRectangle } from "../src/delatin.js";
...
describe("triangulateRectangle", () => {
  it("builds the unit square (same as the default seed)", () => {
    const s = triangulateRectangle(0, 0, 1, 1);
  ...
  it("builds a clamped horizontal band", () => {
    const s = triangulateRectangle(0, 0.25, 1, 0.75);
  ...
  it("is a valid reprojector seed (converges, stays in the band)", () => {
    const r = new RasterReprojector(fns, 64, 64, {
      initialTriangulation: triangulateRectangle(0, 0.2, 1, 0.8),
    });
    r.run(0.125);
