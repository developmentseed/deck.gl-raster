...
_Some projections may render with some distortion_. This initial implementation takes a simpler rendering approach, using a fixed number of resampling points, instead of the more accurate dynamic resampling approach used in the standard Web Mercator [`MapView`]. Create an issue if you have data that is rendering incorrectly.
