...
Our implementation uses the same shape: cogeotiff's `image.getTileSize(idx)` reads `TileOffsets[idx]` and `TileByteCounts[idx]` lazily through the wrapped header source, and our chunk cache turns those 4–8 byte reads into shared 64 KiB block fetches. We explicitly pass `prefetch: chunkSize` to `Tiff.create` so the very first byte read is a full block too (cogeotiff's default `DefaultReadSize` is only 16 KiB).
